...
saveEventBtn.addEventListener('click', async () => {
    ...
    if (category === 'NEW') {
        ...
        if(!categoriesData.includes(category)) {
            ...
            // Additional styling injections for custom categories are currently handled in app.js on load,
            // so we don't strictly *need* to duplicate the color-generation logic here, 
            // since this is just adding it to the database.
        }
    }
    ...
    const newEvent = {
        ...
        price,
        description,
        baseGoing: Math.floor(Math.random() * 50) + 1, // Start with a few fake RSVP
        image
    };
